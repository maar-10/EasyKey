...
- **All-or-nothing.** Every replacement is downloaded and checksum-verified into a `.eknew`
  ...
  dropped connection, a chunk unload or a mid-publish 404 leaves your install exactly as it
  was. This is the failure mode the self-extracting installers have and this cannot.
- **Config is sacred.** Nothing matching the protected list is ever written, moved or deleted
  — your keys, approvals, panel and lift definitions, device identity and pinned server. The
  check is asserted immediately before each write, so the guarantee doesn't depend on the
  manifest being correct.
- **It warns before it costs you anything.** Replacing an `easykey/config.lua` you have
  hand-edited copies it to `/easykey_backup/<timestamp>/` and says so. If a release changes
  the persisted-config format, it backs up **every** `/easykey_*.cfg` first and tells you your
  settings may need re-entering.

Also worth knowing:

- The **first** run after an update is fast: `/easykey_version.txt` records the installed
  release, and a matching version skips checksumming ~250 KB of vendored crypto entirely. Use
...
existing completeness guard protects both. Each entry carries the file's byte size and an
FNV-1a checksum. HTTPS to the pinned `raw.githubusercontent.com` URL is the trust root, exactly
as it is for `wget run`; the checksums answer "did this change" and "did the download arrive
intact" — they are not a signature.
...
that config survives, that a failed download changes *nothing*, and that the Lua checksum agrees
with the Node generator's (if those two ever drift, every file looks permanently outdated).
