--- Scenario test for easykey_update.lua, driven against a REAL http server.
---
...
--- local mirror of the repo (tests/run_updater.sh serves it).
---
...
--- Run the updater, capturing everything it prints (its output IS its interface).
local function runUpdater(...)
    local args = { ... }
    local captured = {}
    local realWrite = term.write
    local nativeTerm = term.current()
    -- redirect into a window so prints don't scroll the real screen away
    local w, h = term.getSize()
    local win = window.create(nativeTerm, 1, 1, w, math.max(h, 30), false)
    term.redirect(win)
    local ok, err = pcall(function() shell.run("/easykey_update.lua", table.unpack(args)) end)
    term.redirect(nativeTerm)
    for y = 1, math.max(h, 30) do
...
-- =====================================================================
-- Scenario 0: no role installed
-- =====================================================================
log("")
log("SCENARIO 0  no role.txt -> refuses politely, installs nothing")
if fs.exists("/role.txt") then fs.delete("/role.txt") end
local o0 = runUpdater()
check(contains(o0, "No EasyKey role is installed"), "says no role is installed")
check(contains(o0, "easykey_update.lua <role>"), "tells you how to name one")
check(not fs.exists("/easykey"), "wrote nothing")

-- =====================================================================
-- Scenario 1: fresh install by naming a role
-- =====================================================================
log("")
log("SCENARIO 1  named role -> full fresh install")
...
writeFile("/easykey_update_src.txt", corruptBase)
local o6, ok6 = runUpdater("--verify")
writeFile("/easykey_update_src.txt", BASE)
