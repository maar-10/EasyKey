...
DATA="${TMPDIR:-/tmp}/easykey_ui_manual/data"
C0="$DATA/computer/0"
rm -rf "$C0"; mkdir -p "$C0"
...
cp "$EASYKEY/tests/ui_render_manual.lua" "$C0/startup.lua"
rm -f "$C0/results.txt"
timeout 60 "$CRAFTOS" --headless -d "$DATA" >/dev/null 2>&1 || true
echo "=== results.txt ==="; cat "$C0/results.txt" 2>/dev/null || echo "(no results)"
for f in ui_manual_panel ui_manual_devices ui_manual_sessions ui_manual_use ui_manual_use_denied; do
